...
COVID 19 has affected everyone around the globe, and it still remains to be a major playing factor in everyone's lives.
We interact and are involved with many industries throughout our lives 
...
title("Daily Confirmed Covid Cases from 2020 to 2022")
```
Before we start, here is the adjusted COVID case plot over time matched with the associated stock price dates (this is because stocks opening prices are measured only 5 days a week).
...
means that it has practically no correlation. Besides this sector, the other sectors and the vaccines have a somewhat moderate positive correlation to COVID cases over time. In particular, it appears that Johnson&Johnson have the weakest correlation out of the three vaccine types, while Moderna is the strongest, though not by a drastic difference.

Below are plots that represent these associations, in visual form.
